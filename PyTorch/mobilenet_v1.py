    ...
    def __init__(self, num_classes: int, alpha: float = 1.0):
        ...
        if not use_alpha:
            self.mobile_net = Sequential(OrderedDict([
                ('conv1', Conv2d(1, 32, kernel_size=3, stride=2, padding=1)),
                ('bn', BatchNorm2d(32)),
                ('relu', ReLU(inplace=True)),
                ...
                ('conv9', DWSepConv(512, 1024, stride=2, padding=2)),
                ('conv10', DWSepConv(1024, 1024)),
                ('avg_pool', AvgPool2d(kernel_size=1)),
                ('flatten', Flatten(start_dim=1)),
                ('fc', Linear(1024 * 4, num_classes))
            ]))
